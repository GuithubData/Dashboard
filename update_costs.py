"""
update_costs.py — يشتغل مرتين بالسنة تلقائياً عبر GitHub Actions
(.github/workflows/costs-refresh.yml) — بدون أي تدخل يدوي.
...
DATASET_B_FILE = Path(__file__).parent / "dataset_b.json"
OUTPUT_FILE = Path(__file__).parent / "costs.json"
...
def main():
    ...
    print(f"\n✓ انكتب costs.json — {len(facts)} صندوق من أصل {len(isins)} "
