"""
sync_notes.py — يترجم الوصف (description) الإنجليزي الموجود أصلاً
بـ dataset_b.json (يلي sync_dataset_b.py بيجيبه أصلاً من overview.get("description"))
للعربي، ويخزن الترجمة بس بملف notes_ar.json — بدون أي جلب جديد من justETF
(ما في داعي، الوصف الإنجليزي موجود أصلاً بالـ snapshot).

⚠️ لازم يشتغل بعد sync_dataset_b.py (بنفس الـ workflow، كخطوة تالية له) عشان يلاقي
أحدث نسخة من dataset_b.json.
...
DATASET_FILE = Path("dataset_b.json")
OUTPUT_FILE = Path("notes_ar.json")
REQUEST_DELAY = 0.8
...
def main():
    if not DATASET_FILE.exists():
        print("dataset_b.json غير موجود — شغّل sync_dataset_b.py الأول.")
        return

    snapshot = json.loads(DATASET_FILE.read_text(encoding="utf-8"))
