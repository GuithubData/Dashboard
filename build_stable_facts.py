"""
build_stable_facts.py — يشتغل مرة/سنة تلقائياً عبر GitHub Actions
(.github/workflows/build-stable-facts.yml) — بدون أي تدخل يدوي.
...
هاد السكربت بيسحب:
  ...
  2. justetf_snapshot.json المتراكم بنفس هالريبو (ناتج scrape_justetf.py)
ويحسب الحقول السبعة لكل ISIN موجود بـ isins.txt، ويكتبهم لملف
stable_fund_facts.json — يلي الـ workflow بيعمله commit تلقائياً،
وetfdb-proxy.php عالموقع بيسحبه من GitHub raw (نفس نمط justetf_snapshot).
"""
import json
...
JUSTETF_SNAPSHOT_FILE = Path(__file__).parent / "justetf_snapshot.json"  # نفس الملف يلي scrape_justetf.py بيبنيه بهالريبو
OUTPUT_FILE = Path(__file__).parent / "stable_fund_facts.json"
...
def main():
    ...
    print("قراءة justetf_snapshot.json المحلي (نفس الريبو)...")
    j_funds = {}
    ...
    print(f"\n✓ انكتب stable_fund_facts.json — {len(facts)} صندوق من أصل {len(isins)}")
    if missing:
